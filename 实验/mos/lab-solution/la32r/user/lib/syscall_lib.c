...
int syscall_read_block(u_int blkno, void *va) {
	return msyscall(SYS_read_block, blkno, va);
}

int syscall_write_block(u_int blkno, const void *va) {
	return msyscall(SYS_write_block, blkno, va);
}
