...
static void map_segment(Pde *pgdir, u_int asid, u_long pa, u_long va, u_int size, u_int perm) {
	...
	for (int i = 0; i < size; i += PAGE_SIZE) {
		...
		// 把虚拟地址va+i映射到物理地址pa+i，并设置标志位为perm
		// 使用panic_on来寻求可能的报错
...
int envid2env(u_int envid, struct Env **penv, int checkperm) {
	...
	// 如果envid为0，设置penv为当前进程并返回
	// 否则获取id对应的进程。其中ENVX宏用于获取数组索引
	if (envid == 0) {
		*penv = curenv;
		return 0;
	}
	...
	// checkperm不为0，e既不是curenv也不是curenv的子进程
...
void env_init(void) {
	...
	for (i = NENV - 1; i >= 0; --i) {
		// 题目要求，使用倒叙进行插入
		envs[i].env_status = ENV_FREE; // 设置状态为空闲
		LIST_INSERT_HEAD(&env_free_list, &envs[i], env_link); // 把envs[i]插入到链表env_free_list
	}
...
int env_alloc(struct Env **new, u_int parent_id) {
	...
	LIST_REMOVE(e, env_link); // 申请完PCB后，把其在空闲链表中移除
